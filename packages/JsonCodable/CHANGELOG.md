...
- `@JsonCodable` 对 `public` / `package` 类型生成同级访问修饰的 `init(from:)` / `encode(to:)`
...
- `dlog` 复用静态 `DateFormatter`；README 澄清 `defaultValue` 不覆盖 JSON `null`、`@CodingKey` 需配合 `@JsonCodable`
...
- `@JsonCodable` / `@CodingKey` 宏，支持 JSON key、别名与解码默认值
