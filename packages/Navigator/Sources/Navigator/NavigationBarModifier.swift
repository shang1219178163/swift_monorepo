...
//  title 为 nil 时可选回落环境中 `Navigator.titleProvider`。
//
...
import SwiftUI
    ...
    {
        /// 显式标题；为 nil 时尝试 `navigator.titleProvider`
        ...
        private var resolvedTitle: String {
            ...
            guard let name = currentRoute?.name else { return "" }
            return navigator.titleProvider?(name) ?? name
        }

        private var hasCustomTitle: Bool { TitleContent.self != EmptyView.self }
        private var hasLeadingExtra: Bool {
            LeadingExtra.self != EmptyView.self
        }
        ...
        public func body(content: Content) -> some View {
            content
                ...
                .tint(barTint)
                .toolbarBackground(
                    backgroundColor ?? defaultBarBackground,
                    for: .navigationBar
                )
    ...
    extension View {
        ...
        /// - Parameter title: 为 nil 时回落 `Navigator.titleProvider`（依赖 `\.currentRoute`）。
        public func navigationBarCustom<
            TitleContent: View,
            LeadingExtra: View,
            Trailing: View
        >(
