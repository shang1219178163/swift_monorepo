...
public final class Get {
    ...
    /// 导航栏标题回落（转发到当前引擎；未 `setup` 时为 `nil`）
    public static var titleProvider: RouteTitleProvider? {
        engine?.titleProvider
    }
    
    /// 已注入的引擎；未 `setup` 前访问会 preconditionFailure
    public static var shared: Navigator {
        guard let engine else {
            preconditionFailure("Call Get.setup(...) before use")
        }
        return engine
    }
    
    /// 清空引擎（测试 / Preview 重建）。会先结束未决 `await`，之后须重新 `setup`。
    public static func reset() {
        engine?.finishPendingWaits()
        engine = nil
    }
    ...
    /// - 首次创建引擎；`tabCount` 不同则先结束未决等待再重建；策略闭包 / unknown / 标题回落始终覆盖。
    public static func setup(
        ...
        if engine == nil {
            ...
            engine?.containsRoute = containsRoute
            engine?.preventsDuplicate = preventsDuplicate
            engine?.titleProvider = titleProvider
    ...
    @discardableResult
    public static func addListener(_ handler: @escaping RouteChangedHandler) -> RouteListenerID {
        shared.addListener(handler)
    }
    
    @discardableResult
    public static func addTabListener(_ handler: @escaping TabChangedHandler) -> RouteListenerID {
        shared.addTabListener(handler)
    }
