...
final class NavigatorTests: XCTestCase {
    ...
    func testGetSetupStoresUnknownRoute() {
        Get.setup(
            tabCount: 2,
            containsRoute: { $0.hasPrefix("/") },
            unknownRoute: "/unknown"
        )
        XCTAssertEqual(Get.shared.unknownRoute, "/unknown")
        Get.reset()
    }
    
    func testResetCompletesPendingPush() async {
        Get.setup(
            tabCount: 1,
            containsRoute: { _ in true },
            unknownRoute: "/unknown"
        ...
        let box = AwaitBox()
        Task { @MainActor in
            box.result = await Get.toNamed("/detail")
            box.finished = true
        }
        await waitUntil { Get.shared.canPop }
        Get.reset()
        _ = await awaitFinish(box)
        XCTAssertNil(box.result)
    }
    
    func testSetupUpdatesContainsRoute() async {
        Get.setup(
            tabCount: 1,
            containsRoute: { $0 == "/a" || $0 == "/unknown" },
            unknownRoute: "/unknown"
        ...
        Get.setup(
            tabCount: 1,
            containsRoute: { $0 == "/b" || $0 == "/unknown" },
            unknownRoute: "/unknown"
        )
        let box = startPush(Get.shared, "/b")
        await waitUntil { Get.shared.routes == ["/b"] }
        Get.shared.pop()
        _ = await awaitFinish(box)
        
        let fallback = startPush(Get.shared, "/a")
        await waitUntil { Get.shared.routes == ["/unknown"] }
        XCTAssertEqual(
            Get.shared.currentArgs?[NavigatorArgKey.intendedRoute] as? String,
            "/a"
        )
        Get.shared.pop()
        _ = await awaitFinish(fallback)
    }
    
    func testSelectedTabNotifiesTabListenersNotRoute() async {
        let navigator = Navigator(
            tabCount: 2,
            containsRoute: { _ in true },
            unknownRoute: "/unknown"
        ...
        var tabChanges: [(Int, Int)] = []
        _ = navigator.addListener { _, to in
            routeNames.append(to?.name ?? "root")
        }
        _ = navigator.addTabListener { from, to in
            tabChanges.append((from, to))
        }
        let box = startPush(navigator, "/tab0")
        await waitUntil { navigator.routeName == "/tab0" }
        XCTAssertEqual(routeNames, ["/tab0"])
        
        navigator.selectedTab = 1
        XCTAssertEqual(tabChanges.last?.0, 0)
        XCTAssertEqual(tabChanges.last?.1, 1)
        XCTAssertEqual(navigator.routeName, nil)
        XCTAssertEqual(routeNames, ["/tab0"])
        
        navigator.selectedTab = 0
        XCTAssertEqual(tabChanges.last?.0, 1)
        XCTAssertEqual(tabChanges.last?.1, 0)
        XCTAssertEqual(navigator.routeName, "/tab0")
        XCTAssertEqual(routeNames, ["/tab0"])
    ...
    func testUnknownPreventDuplicatesAllowsDifferentIntended() async {
        let navigator = Navigator(
            tabCount: 1,
            containsRoute: { $0 == "/unknown" },
            preventsDuplicate: { _ in true },
            unknownRoute: "/unknown"
        )
        let first = startPush(navigator, "/a")
        await waitUntil { navigator.canPop }
        let second = startPush(navigator, "/b")
        await waitUntil { navigator.routes.count == 2 }
        XCTAssertEqual(navigator.routes, ["/unknown", "/unknown"])
        XCTAssertEqual(
            navigator.currentArgs?[NavigatorArgKey.intendedRoute] as? String,
            "/b"
        )
        navigator.pop()
        navigator.pop()
        _ = await awaitFinish(first)
        _ = await awaitFinish(second)
    }
